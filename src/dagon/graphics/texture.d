...
class Texture: Owner
{
    ...
    this(SuperImage img, Owner o, bool genMipmaps = true, bool bilinearFiltering = false)
    {
        super(o);
        translation = Vector2f(0.0f, 0.0f);
        scale = Vector2f(1.0f, 1.0f);
        rotation = 0.0f;
        createFromImage(img, genMipmaps, bilinearFiltering);
    }

    void createFromImage(SuperImage img, bool genMipmaps = true, bool bilinearFiltering = false)
    {
        if (glIsTexture(tex))
        ...
        glTexImage2D(GL_TEXTURE_2D, 0,
            intFormat, width, height, 0, format,
            type, cast(void*)img.data.ptr);
    }

    void updateFromImage(SuperImage img)
    {
        glBindTexture(GL_TEXTURE_2D, tex);
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, format, type, cast(void*)img.data.ptr);
